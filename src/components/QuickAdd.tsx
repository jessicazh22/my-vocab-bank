...
export default function QuickAdd({ onClose, addWord, checkDuplicate, locale = 'en-AU' }: QuickAddProps) {
  ...
  const enrichWord = async (word: string, isPhrase: boolean) => {
    try {
      ...
      return {
        ...
        distinction: data.distinction as string | null | undefined,
      };
    } catch {
      return {};
    }
  ...
  const doAddWord = async (inputWord: string, definition?: string) => {
    ...
    let aiDistinction: string | undefined;
    ...
    if (!definition) {
      ...
      aiDistinction = enriched.distinction || undefined;
