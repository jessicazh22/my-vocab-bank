...
export default function OnboardingSession({ words, practiceWord, onClose }: Props) {
  ...
  const [selectedUsefulness, setSelectedUsefulness] = useState<Usefulness>(DEFAULT_USEFULNESS);
  const [saving, setSaving] = useState(false);
  const [done, setDone] = useState(false);
  const [results, setResults] = useState<Array<{ word: string; rating: Rating; days: number }>>([]);

  // Reset usefulness to default on word change
  useEffect(() => {
    setSelectedUsefulness(DEFAULT_USEFULNESS);
  }, [currentIndex]);
  ...
  const handleRate = async (rating: Rating) => {
    ...
    setSaving(true);
    try {
      await practiceWord(currentWord.id, '', rating, selectedUsefulness);
      const days = computeDays(rating, selectedUsefulness);
  ...
  const progress = currentIndex / allWords.length;
    ...
    <div className="fixed inset-0 bg-zinc-900 z-50 flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="max-w-lg mx-auto px-6 py-10">
          ...
          {/* Word + definition always visible */}
          <div className="mb-6">
          ...
          <div className="bg-zinc-800/50 border border-zinc-700 rounded-lg p-5 space-y-3 mb-6">
          ...
          {/* Usefulness — default pre-selected, tap to change */}
          <div className="mb-6">
            <p className="text-xs text-zinc-500 uppercase tracking-wider mb-2">How often would you use this?</p>
          ...
          <div>
            <p className="text-xs text-zinc-500 uppercase tracking-wider mb-2">How well do you know it?</p>
              ...
              {(['again', 'hard', 'good', 'easy'] as Rating[]).map(r => {
                ...
                return (
                  <button
                    ...
                    disabled={saving}
                    className={`py-2.5 rounded-lg text-xs font-medium transition-colors disabled:opacity-50 flex flex-col items-center gap-0.5 ${RATING_COLORS[r]}`}
                  >
